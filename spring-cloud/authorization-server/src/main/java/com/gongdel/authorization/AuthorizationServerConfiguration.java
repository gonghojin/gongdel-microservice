...
class UserConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http
				.authorizeRequests()
					.antMatchers("/actuator/**").permitAll() // url 매핑
					.mvcMatchers("/.well-known/jwks.json").permitAll() // mvc 패턴
...
/**
 * 레거시 인증 서버는 user parameter 에 대한  custom name 을 지원하지 않기 떄문에 기본값 확장이 필요하다.
 * 기본적으로 user_name 속성을 사용하지만,  jwt 사양에 정의된 하위 속성(sub)을 사용하는 게 좋다
 */
